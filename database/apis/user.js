...
async function createUserTable(req, res) {
    db = await openDB();
      await db.run(`CREATE TABLE IF NOT EXISTS ${tableName}(UUID unique, user, mail, is_authenticated boolean)`,(err, result) => {
        if(err) {
          console.log("err")
  ...
  function createUser(req, res, db) {
      ...
      console.log("AUTH","is_authenticated" in tableValues)
